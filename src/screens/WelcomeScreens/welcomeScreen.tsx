...
const WelcomeScreen = () => {
  ...
  const getCurrentScreenId = useCallback((screensArray: WelcomeScreenState[]) => {
  ...
  const showNextScreenAnimation = (currentScreenId: number) => {
    'worklet';
    let fullWidth = (measureMents.windowWidth - marginConst * (noOfScreens.length + 1)) /
    noOfScreens.length
        ...
        {noOfScreens.map(eachScreen => (
          <View
            style={{
              flex: 1,
              display:
                getCurrentScreenId(noOfScreens) === eachScreen.id ? 'flex' : 'none',
            }}
            key={eachScreen.id}>
            <TouchableOpacity
              activeOpacity={0.5}
            ...
            <View style={styles.heavyTextContainer}>
              ...
              <ButtonComponent onPress={showNextScreenClick}>Next</ButtonComponent>
              <TouchableOpacity
                onPress={onSkipClick}
                style={{
                  alignSelf: 'center',
